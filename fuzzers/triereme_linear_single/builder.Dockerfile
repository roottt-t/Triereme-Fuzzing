...
RUN git clone --recursive --branch main \
    https://github.com/vusec/triereme.git /mine

RUN cd /mine && \
    unset CFLAGS && unset CXXFLAGS && \
    cargo build --release --manifest-path libafl_tracer/Cargo.toml && \
    mkdir "$OUT/lib" && \
    cp libafl_tracer/target/release/libSymRuntime.so "$OUT/lib" && \
    cargo clean --manifest-path libafl_tracer/Cargo.toml && \
    cargo build --release --manifest-path fuzzing_helper/Cargo.toml && \
    cp target/release/fuzzing_helper "$OUT" && \
    cargo clean --manifest-path fuzzing_helper/Cargo.toml
